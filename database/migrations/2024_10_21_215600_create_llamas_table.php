...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('llamas', function (Blueprint $table) {
            ...
            $table->string('id_clase', 20)->nullable()->references('id_clase')->on('clases');
            $table->string('id_criador', 20)->nullable()->references('id_criador')->on('criadores');
            ...
            $table->string('id_color_pelaje', 20)->nullable()->references('id_color_pelaje')->on('colores_pelaje');
            $table->string('id_color_ojos', 20)->nullable()->references('id_color_ojos')->on('colores_ojos');
            $table->string('id_denticion', 20)->nullable()->references('id_denticion')->on('denticiones');
            $table->integer('numero_partos')->nullable();
            $table->date('fecha_ultimo_parto')->nullable();
            $table->string('id_estado_reproductivo', 20)->nullable()->references('id_estado_reproductivo')->on('estados_reproductivos');
            $table->string('id_estado_llama', 20)->nullable()->references('id_estado_llama')->on('estados_llama');
